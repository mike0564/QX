function  calendar() {
    ...
    var calendar = {
      ...
      verifyTime: function verifyTime(date) {
        splitData =  date.split("-");
        var y = parseInt(splitData[0]);
        var m = parseInt(splitData[1]);
        var d = parseInt(splitData[2]);
        //年份限定、上限
        if (y < 1900 || y > 2100) {
        ...
        if(d > this.solarDays(y, m)) {
          return false;
        }
        return true;
      },
      daysBetween: function daysBetween(obj) {
        const start = new Date(obj.cYear, obj.cMonth - 1, obj.cDay);
        const end = new Date();
        return Math.ceil((end - start) / (1000 * 60 * 60 * 24));
      },
      birthday: function birthday(y, m, d, type) {
        // 获取当前日期 阳历
        const today = new Date();
        const currentYear = today.getFullYear();
        let nextBirthdayCalendar;
        let nextBirthdayYear;
        let nextBirthdayMonth;
        let nextBirthdayDate;

        if(type==="true"||type===true) {
          //农历转阳历
          nextBirthdayCalendar = this.lunar2solar(currentYear, m, d);
          nextBirthdayYear = nextBirthdayCalendar.cYear;
          nextBirthdayMonth = nextBirthdayCalendar.cMonth;
          nextBirthdayDate = nextBirthdayCalendar.cDay;
        }else{
          //阳历,无需转换
        ...
        // 如果今年的生日已经过去，年份设置为明年，重新计算阳历日期
        if (today > new Date(nextBirthdayYear, nextBirthdayMonth-1, nextBirthdayDate)) {

          if(type==="true"||type===true) {
            //农历转阳历
            nextBirthdayCalendar = this.lunar2solar(currentYear+1, m, d);
            nextBirthdayYear = nextBirthdayCalendar.cYear;
            nextBirthdayMonth = nextBirthdayCalendar.cMonth;
            nextBirthdayDate = nextBirthdayCalendar.cDay;
          }else{
        ...
        const nextBirthday = new Date(nextBirthdayYear, nextBirthdayMonth-1, nextBirthdayDate);
      ...
      getAnimalZodiacToEmoji: function getAnimalZodiacToEmoji(animal) {
        const map = {
          '鼠': '🐭', '牛': '🐮', '虎': '🐯', '兔': '🐰',
          '龙': '🐲', '蛇': '🐍', '马': '🐴', '羊': '🐑',
          '猴': '🐵', '鸡': '🐔', '狗': '🐶', '猪': '🐷',
